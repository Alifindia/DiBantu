...
@api_router.get("/")
async def root():
    return {"message": "TemuJasa API - Service Marketplace"}
...
async def create_bantuin_request(input: BantuinCreate):
    ...
    if not is_valid:
        if violation_type == "technical":
            raise HTTPException(
                status_code=400, 
                detail="Layanan ini tersedia di kategori utama TemuJasa. Silakan pilih kategori yang sesuai agar ditangani oleh teknisi profesional."
            )
        else:  # safety
            raise HTTPException(
                status_code=400, 
